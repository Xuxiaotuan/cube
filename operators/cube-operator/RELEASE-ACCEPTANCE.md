...
### Independently approved directed edges

Existing-installation acceptance now also requires `--from-lock-sha256 APPROVED-OLD-LOCK-SHA --from-root OLD-ARTIFACT-ARCHIVE --edges APPROVED-EDGES.json --edges-sha256 INDEPENDENTLY-APPROVED-REGISTRY-SHA`. The checker verifies both inventories, rejects a same-release "upgrade", and requires exactly one directed edge for the selected transition. It does not infer edges from version strings, RPC names, digest equality or a manifest `verified: true` claim. The old archive is separate from the new checkout so changed source/CRD/config/test files can be checked honestly.

The registry has `schema: 1` and an `edges` array. Each approved entry contains `from_release_id`, `to_release_id`, `transition` (`upgrade` or `rollback`), `mode`, `source_lock_sha256`, `target_lock_sha256`, and `evidence_sha256` (the exact evidence index file). These hashes bind the independently approved entry to both immutable lock files and all reports/raw evidence transitively. There are deliberately NO real entries supplied by this change. Approval must happen outside the editable artifact directory, after the release owner reviews actual tests. A caller who controls the approval hash still controls trust; this is not cryptographic attestation of runtime truth or a deployment authorization system.

The evidence index must say `installation: existing`. The main transition report additionally contains `old_observed_images` matching the source lock and `tested_image_sets`, a list of complete role-to-image maps observed during the test. Source and target maps are mandatory; every image must belong to the approved source/target pair for that role. `mixed-version` additionally requires a genuinely mixed observed map and a `mixed-version-rpc` report. `fenced-maintenance` instead requires `maintenance-fencing` evidence. These extra reports carry the same exact release pair and normal raw/data assertions. An observed map alone does not prove all RPCs or intermediate combinations were exercised: the approving owner must assess operation/state coverage, old Worker draining, fencing and result visibility. Final report `observed_images` still represents the converged target.

Reports now require timezone-qualified ISO8601 timestamps in chronological order, and assertion comparison distinguishes a boolean from a numeric count. Production RPO/RTO values must be finite; no default budgets are invented. The gate verifies supplied evidence, not runtime protocol negotiation. Runtime admission/handshake enforcement remains absent and MUST NOT be reported as fixed by this registry.

To establish the first real edge, supply the actual supported old release's Operator/agent/API/Rust image digests and pullable image artifacts, old CRDs/config/test/source inventory with approved hashes, persisted MetaStore + CacheStore + referenced object snapshots, and a controlled environment capable of restoring them. Run old-to-new mixed RPC or fenced maintenance, interruption, partial failure, stale attempt and persistent-state-readability tests. For rollback, separately supply the reverse edge with state WRITTEN by the new version; successfully reading only a pre-upgrade backup does not prove reverse compatibility. Missing old binaries or state fixtures is `external_blocked`, not permission to manufacture an approved edge.
