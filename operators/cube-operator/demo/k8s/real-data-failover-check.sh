...
MYSQL_READ_TIMEOUT_SECONDS="${MYSQL_READ_TIMEOUT_SECONDS:-30}"
MYSQL_WRITE_TIMEOUT_SECONDS="${MYSQL_WRITE_TIMEOUT_SECONDS:-30}"
...
q() {
  ...
  "$KUBECTL" run -n "$NAMESPACE" "$client_pod" --rm -i --quiet --restart=Never \
    ...
    mysql --protocol=TCP --connect-timeout="$MYSQL_CONNECT_TIMEOUT_SECONDS" \
      --read-timeout="$MYSQL_READ_TIMEOUT_SECONDS" --write-timeout="$MYSQL_WRITE_TIMEOUT_SECONDS" \
